...
export const securityLayers: SecurityLayer[] = [
  {
    ...
    routeUseCase: "Protects export, profile, support, and reporting routes that return user or customer records.",
    reusableDatasets: ["Customer profiles", "KYC records", "Support tickets", "Billing contacts", "CSV exports"],
    vulnerabilities: ["PII in responses", "Unmasked exports", "Overshared profile fields", "Sensitive logs"],
    setupSteps: ["Select the route patterns", "Map sensitive dataset fields", "Choose redaction or block mode", "Verify with a sample response"],
  ...
  },
  {
    ...
    category: "Identity",
    routeUseCase: "Protects private dashboards, admin APIs, user sessions, and organization-scoped resources.",
    reusableDatasets: ["User sessions", "Admin records", "Organization memberships", "Permission maps", "Audit events"],
    vulnerabilities: ["Missing auth checks", "Scope bypass", "Expired token reuse", "Role drift"],
    setupSteps: ["Add trusted issuers", "Attach required scopes", "Bind routes to roles", "Run an access simulation"],
    healthStatus: "healthy",
    enabled: true,
    severity: "critical",
    plan: "Growth",
  ...
  },
  {
    ...
    routeUseCase: "Protects login, search, checkout, scraping-prone, and public API routes from abnormal volume.",
    reusableDatasets: ["Login attempts", "Search indexes", "Checkout sessions", "Public API records", "Catalog data"],
    vulnerabilities: ["Burst scraping", "Credential stuffing", "Endpoint abuse", "IP rotation"],
    setupSteps: ["Pick route groups", "Set request windows", "Choose block duration", "Tune trusted IP ranges"],
    healthStatus: "healthy",
    enabled: true,
    severity: "high",
    plan: "Starter",
  ...
  },
  {
    ...
    routeUseCase: "Protects import, webhook, template, form submission, and GraphQL mutation routes.",
    reusableDatasets: ["Webhook payloads", "Form submissions", "Import batches", "Template inputs", "GraphQL mutations"],
    vulnerabilities: ["SQL fragments", "Template injection", "Encoded payloads", "Binary anomalies"],
    setupSteps: ["Define inspected methods", "Set scan depth", "Add allowlisted fields", "Enable block-on-critical"],
  ...
  },
  {
    ...
    category: "Secrets",
    routeUseCase: "Protects debug, webhook, integration, observability, and internal tooling routes.",
    reusableDatasets: ["Debug traces", "Webhook secrets", "Integration tokens", "Service credentials", "Log streams"],
    vulnerabilities: ["Token echoing", "Private key exposure", "Secret leakage", "Unsafe debug mirrors"],
    setupSteps: ["Register secret patterns", "Choose revoke policy", "Enable log scrubbing", "Test with fingerprinted samples"],
    healthStatus: "healthy",
    enabled: true,
    severity: "critical",
    plan: "Enterprise",
  ...
  },
  {
    ...
    routeUseCase: "Protects browser-facing APIs, preview services, docs portals, and session refresh routes.",
    reusableDatasets: ["Browser sessions", "Preview APIs", "Docs content", "Session refresh data", "Partner portals"],
    vulnerabilities: ["Wildcard origins", "Credentialed broad access", "Unsafe preflights", "Origin drift"],
    setupSteps: ["Submit allowed origins", "Review credential policy", "Set preflight cache", "Monitor drift alerts"],
  ...
  },
  {
    ...
    category: "Transport",
    routeUseCase: "Protects static docs, auth callbacks, browser apps, and API routes with strict transport policy.",
    reusableDatasets: ["Docs responses", "Auth callbacks", "Static assets", "Browser routes", "API metadata"],
    vulnerabilities: ["Missing CSP", "Weak cache control", "HSTS gaps", "Header drift"],
    setupSteps: ["Choose required headers", "Set CSP mode", "Define cache rules", "Verify route coverage"],
    healthStatus: "healthy",
    enabled: true,
    severity: "medium",
    plan: "Starter",
  ...
  },
  {
    ...
    routeUseCase: "Protects high-value APIs by prioritizing the routes most likely to expose critical data.",
    reusableDatasets: ["Endpoint inventory", "Risk scores", "Exploit attempts", "Route ownership", "Service maps"],
    vulnerabilities: ["High-risk clusters", "Route drift", "Weakness density", "Exploit pressure"],
    setupSteps: ["Import route inventory", "Assign business criticality", "Set scoring weights", "Review risk ranking"],
    healthStatus: "healthy",
    enabled: true,
    severity: "high",
    plan: "Growth",
  ...
  },
  {
    ...
    category: "Data Protection",
    routeUseCase: "Protects analytics, audit, export, support, and internal routes that return sensitive fields.",
    reusableDatasets: ["Audit exports", "Analytics rows", "Support views", "Finance records", "Internal traces"],
    vulnerabilities: ["Unmasked identifiers", "Observability leaks", "Unsafe export formats", "Masking bypass"],
    setupSteps: ["Select field policy", "Bind datasets", "Preview response mutations", "Enable log masking"],
    healthStatus: "degraded",
    enabled: true,
    severity: "medium",
    plan: "Growth",
  ...
  },
  {
    ...
    routeUseCase: "Protects sensitive routes where unusual origin, payload, or timing patterns indicate abuse.",
    reusableDatasets: ["Request baselines", "Origin fingerprints", "Upload payloads", "Session paths", "Geo signals"],
    vulnerabilities: ["Origin drift", "Payload entropy spikes", "Request outliers", "Unusual access timing"],
    setupSteps: ["Set warmup window", "Choose sensitivity", "Enable geo signals", "Review first baseline"],
    healthStatus: "healthy",
    enabled: true,
    severity: "high",
    plan: "Enterprise",
